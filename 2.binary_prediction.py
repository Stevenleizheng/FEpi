...
def argument():
    ...
    parser.add_argument('-t', '--threshold', type=float, default=0.6, help='threshold,(eg. 0.6)')
...
def prediction(strgpu, data_path, batch_size, output,
    ...
    with torch.no_grad():
        ...
        for data, _ in tqdm(dataloader):
            ...
            prompt = torch.cat((prompt, prediction[0].cpu()), dim=0)
            pred = F.softmax(prediction[1],1).cpu().detach().numpy()
        ...
        torch.save(prompt.clone(), os.path.join(data_path, "enzyme_prompt.pt"))
...
def main():
    ...
    prediction(strgpu=strgpu, data_path=data_path, batch_size=batch_size, output=output,
                param_path=f'{sys.path[0]}/model_param/ft4_MLP_epoch3.pt', threshold=threshold, seed=2024)
